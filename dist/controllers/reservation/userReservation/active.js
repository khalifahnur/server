...
const userActiveReservation = async (req, res) => {
    ...
    try {
        ...
        const responseData = reservations.map((reservation) => {
            ...
            const restaurantDetails = Array.isArray(restaurant.data) && restaurant.data[0]
                ? restaurant.data[0]
                : {};
            return {
                id: reservation._id,
                ...
                date: new Date(reservation.reservationInfo.bookingFor)
                    .toISOString()
                    .split("T")[0],
                time: new Date(reservation.reservationInfo.bookingFor).toLocaleTimeString([], {
                    hour: "2-digit",
                    minute: "2-digit",
                }),
                table: reservation.reservationInfo.tableNumber,
                restaurantName: restaurant.title || restaurant.restaurantName || "Unknown Restaurant",
